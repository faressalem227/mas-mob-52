...
const LocationGrid = ({ DepartmentID }) => {
		...
		const fetchData = async () => {
			...
			try {
				const response = await api.get(
					`/table?sp=ms_SubLocation_Trx&LocationID=${DepartmentID}`
				);
	...
	const confirmAction = async () => {
		try {
			if (modalType === "add") {
				setModelLoader(true);
				const response = await api.post("/table?sp=api_ms_SubLocation_Ins", {
					...rowData,
									...
									<TouchableOpacity
										...
										<Row
											...
											style={[
												...
												selectedRow?.SubLocationID ===
													dataRow?.SubLocationID && {
