...
const WorkOrdersGrid = ({ DepartmentID, TradeID }) => {
	...
	useEffect(() => {
		if (DepartmentID && TradeID) {
			const fetchData = async () => {
				setLoading(true);
				try {
					const response = await api.get(
						`/table?sp=api_ms_workodres_trx&LocationID=${DepartmentID}&TradeID=${TradeID}`
					);
	...
	const handleDoubleClick = (row) => {
		router.navigate({
			pathname: "/WorkOrderDetails",
			params: { ...row, TradeID: TradeID, LocationID: DepartmentID },
		});
	};
									...
									<TouchableOpacity
										...
										<Row
											...
											style={[
												...
												index === selectedRow && { backgroundColor: "#E4EDF2" },
											]}
											textStyle={styles.text}
											widthArr={dynamicWidthArr} // Dynamic widths
