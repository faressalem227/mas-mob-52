...
const StockItemsTable = ({ GroupID }) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await api.get(
					`/table?sp=api_Sc_Items__Trx&CompanyID=1&GroupID=${GroupID}`
				);
	...
	const confirmAction = async () => {
		try {
			if (modalType === "add") {
				setModelLoader(true);
				const response = await api.post("/table?sp=api_Sc_Items_Ins", {
					...rowData,
				...
				setModelLoader(true);
				const response = await api.put(`/table?sp=api_Sc_Items_Upd`, {
					...rowData,
					CompanyID: DepartmentID,
					ItemID: ItemID,
					UnitID: UnitID,
				...
				setModelLoader(true);
				await api.delete(
					`/table?sp=api_ms_FailureReports_Del&LocationID=5&FailureReportID=${selectedRow.FailureReportID}`
				);
				setModelLoader(false);
				setRefetch((prev) => prev + 1);
				setData((prevData) =>
									...
									<TouchableOpacity
										...
										<Row
											...
											style={[
												...
												selectedRow?.ItemID === dataRow?.ItemID && {
