...
const PreventiveMaintenanceContractsGrid = ({ LocationID, ProcedureID }) => {
		...
		if (LocationID) {
			try {
				...
				const response = await api.get(
					`/table?sp=api_ms_Contractors_List&LocationID=5`
				);

				//console.log("Full API Response:", response);
				const List = response?.data?.data || [];
				//console.log("cotractorsssssssssssss", List);
				// Ensure response is an array
				if (Array.isArray(List)) {
					setContractors(List);
					setContractors(
						HandleDropdownFormat("ContractorID", "ContractorName")
					);
				} else {
	...
	const fetchDropdownData = useCallback(async () => {
		if (LocationID && ProcedureID) {
			try {
				...
				// Fetch API response
				const response = await api.get(
					`/table?sp=api_ms_Procedures_Contractors_Trx&LocationID=${LocationID}&ProcedureID=${ProcedureID}`
				);
	...
	const confirmAction = async () => {
		try {
			...
			if (modalType === "add") {
				setModelLoader(true);
				const response = await api.post("/table?sp=api_ms_Contractors_Ins", {
					...rowData,
				...
				setModelLoader(true);
				const response = await api.put(`/table?sp=api_ms_Contractors_Upd`, {
					...rowData,
					LocationID: LocationID,
				});
				setModelLoader(false);
				...
				await api.delete(
					`/table?sp=api_ms_Contractors_Del&LocationID=5&ContractorID=${rowData.ContractorID}`
				);
				setModelLoader(false);
				setRefetch((prev) => prev + 1);
				setContractorsList((prevData) =>
				...
				if (key === "ContractorID") {
					return (
						<Dropdown
							...
							data={Contractors} // Assuming Contractors is populated
							...
							inputName="ContractorID" // Use the key as input name
						/>
					);
				}
			case "date":
...
export default PreventiveMaintenanceContractsGrid;
