...
const AssetCountersGrid = ({ LocationID, SubLocationID, AssetID }) => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const response = await api.get(
					`/table?sp=api_ms_AssetsMeters_Trx&LocationID=${LocationID}&AssetID=${AssetID}`
				);
				setData(response.data.data);
				//console.log("couunters:",response.data.data);
			} catch (err) {
		...
		fetchData();
	}, [LocationID, AssetID]); // Add dependencies here
...
export default AssetCountersGrid;
