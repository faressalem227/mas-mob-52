...
const SpecificInventoryGrid = ({ DepartmentID, data }) => {
	...
	const handelAddNewItem = () => {
		// router.navigate("/AddNewPermissionPage");
		router.navigate("/NewPermitionPage");
	};
	const handleDoubleClick = (row) => {
		//console.log("Double-clicked row", row);

		router.navigate({
			pathname: "/NewPermitionPage",
			params: row,
		});
	};
	...
	const confirmAction = async () => {
		try {
			if (modalType === "add") {
				setModelLoader(true);
				const response = await api.post("/table?sp=api_Sc_Item_Section_Ins", {
					...rowData,
				...
				setModelLoader(true);
				const response = await api.put(`/table?sp=api_Sc_Item_Section_Upd`, {
					...rowData,
					CompanyID: DepartmentID,
				});
				/// GOOD LUCK
				...
				setModelLoader(true);
				await api.delete(
					`/table?sp=api_Sc_Item_Section_Del&CompanyID=${DepartmentID}&SectionID=${selectedRow?.SectionID}`
				);
				setModelLoader(false);
				setRefetch((prev) => prev + 1);
				setData((prevData) =>
									...
									<TouchableOpacity
										...
										<Row
											...
											style={[
												...
												selectedRow?.ProcessID === dataRow?.ProcessID && {
...
export default SpecificInventoryGrid;
