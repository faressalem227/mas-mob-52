...
const ClassificationGrid = ({ DepartmentID, SectionID }) => {
		...
		const fetchData = async () => {
			...
			try {
				const response = await api.get(
					`/table?sp=api_ms_Item_Group_List2&CompanyID=1&SectionID=${SectionID}`
				);
				//console.log(response.data.data,"merna");
	...
	const confirmAction = async () => {
		try {
			if (modalType === "add") {
				//console.log(SectionID,"Marina");

				setModelLoader(true);
				const response = await api.post("/table?sp=api_Sc_Item_Group_Ins", {
					...rowData,
				...
				setModelLoader(true);
				const response = await api.put(`/table?sp=api_Sc_Item_Group_Upd`, {
					...rowData,
					CompanyID: DepartmentID,
				});
				setModelLoader(false);
				...
				setModelLoader(true);
				await api.delete(
					`/table?sp=api_Sc_Item_Group_Del&CompanyID=${DepartmentID}&GroupID=${selectedRow.GroupID}`
				);
				setModelLoader(false);
				//console.log(selectedRow,"Atef");
	...
	return (
		<SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
			<View style={styles.buttonContainer}>
				<CustomButton
					Icon={add_outline}
					title={ MainGrid.addClassBtn[Lang]}
					onPress={handleAdd}
				/>
				<CustomButton
					Icon={PencilLine}
									...
									<TouchableOpacity
										...
										<Row
											...
											style={[
												...
												selectedRow?.GroupID === dataRow?.GroupID && {
