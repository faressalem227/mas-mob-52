...
const PreventiveMaintenanceGrid = ({ LocationID, TradeID }) => {
	const scrollViewRef = useRef(null);

	// const [PreventiveMaintenanceData, setPreventiveMaintenanceData] = useState([]);
	const [PreventiveMaintenanceList, setPreventiveMaintenanceList] = useState(
		[]
	);
	...
	const tableHead = [
		...
		},
		{
			...
			input: "true",
			visible: "true",
			width: 300, // Static width for this column
		},
		{
			key: "Safety",
			label: "مهامات الوقاية و الامان",
			type: "text",
			input: "true",
			visible: "true",
			width: 100, // Static width for this column
	...
	const fetchDropdownData = useCallback(async () => {
		if (LocationID && TradeID) {
			try {
				...
				//console.log("Fetching data for TradeID:", TradeID);
				setLoading(true);

				// Fetch API response
				const response = await api.get(
					`/table?sp=api_ms_Procedures_By_TradeID_Trx&LocationID=${LocationID}&TradeID=${TradeID}`
				);
				...
				if (Array.isArray(List)) {
					setPreventiveMaintenanceList(List);
				} else {
					console.error("Expected List to be an array, but got:", List);
					setPreventiveMaintenanceList([]); // Fallback to empty array
				}
			} catch (err) {
				console.error(
			...
			console.warn("LocationID or TradeID is undefined or invalid.");
		}
	}, [LocationID, TradeID]);
	...
	const fetchProceduresDropdownData = useCallback(async () => {
		try {
			...
			// Fetch API response
			const response = await api.get(`/table?sp=api_ms_Procedures_Types_List`);
	...
	const handleDoubleClick = (row) => {
		router.navigate({
			pathname: "/PreventiveMaintenanceDetails",
	...
	const handleEdit = () => {
		if (selectedRow !== null) {
			setRowData(PreventiveMaintenanceList[selectedRow]);
			setModalType("edit");
			setModalVisible(true);
		} else {
	...
	const confirmAction = async () => {
		try {
			...
			if (modalType === "add") {
				setModelLoader(true);
				const response = await api.post("/table?sp=api_ms_Procedures_Ins", {
					...rowData,
				...
				setPreventiveMaintenanceList((prevData) => [
					...prevData,
					response.data,
				]);
			} else if (modalType === "edit") {
				setModelLoader(true);
				const response = await api.put(`/table?sp=api_ms_Procedures_Upd`, {
					...rowData,
					LocationID: LocationID,
					TradeID: TradeID,
				});
				...
				setModelLoader(true);
				//console.log(rowData.PreventiveMaintenanceID);

				await api.delete(
					`/table?sp=api_ms_Procedures_Del&LocationID=${LocationID}&ProcedureID=${rowData.ProcedureID}`
				);
				setModelLoader(false);
				setRefetch((prev) => prev + 1);
				setPreventiveMaintenanceList((prevData) =>
		...
		<SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
			...
			<ScrollView
				...
				<View className="mb-16">
					...
					<ScrollView>
						<Table>
							{PreventiveMaintenanceList.map((dataRow, index) => (
								<TouchableOpacity
									key={index}
									onPress={() => {
												...
												if (header.input === "true") {
													return (
														<View
															...
															<Text className="text-base text-right font-tmedium font-medium my-2 ">
																{header.label}
															</Text>
															{renderInput(
																header.key,
...
export default PreventiveMaintenanceGrid;
